	...
	ServerCommandHolder COMMAND = new ServerCommandHolder("plumbob", (command, buildContext) -> command
		...
		.then(Commands.literal("set")
			.then(Commands.argument("player", EntityArgument.players())
				.then(Commands.argument("icon", Icon.REGISTERED_DATA_TYPE.argument(buildContext))
					.executes(ctx -> plumbob(EntityArgument.getPlayers(ctx, "player"), Optional.of(Icon.REGISTERED_DATA_TYPE.get(ctx, "icon"))))
				)
			)
		)
