...
public record JoinedZoneShape(List<Ref<ZoneShape>> zoneShapes, AABB box, AAIBB intBox) implements ZoneShape {
	public static final DynamicType<ByteBuf, ZoneShape> TYPE = DynamicType.create(
		"joined",
		"zones",
		ZoneShape.CODEC.listOf(),
		KLibStreamCodecs.listOf(ZoneShape.STREAM_CODEC),
		JoinedZoneShape::create,
		JoinedZoneShape::zoneShapes
	);

	public static JoinedZoneShape create(List<Ref<ZoneShape>> zoneShapes) {
		...
		for (var zoneRef : zoneShapes) {
			var zone = zoneRef.value();
			var box = zone.toAABB();
		...
		return new JoinedZoneShape(zoneShapes, new AABB(minX, minY, minZ, maxX, maxY, maxZ), new AAIBB(iminX, iminY, iminZ, imaxX, imaxY, imaxZ));
	}
	...
	public Stream<BlockPos> getBlocks() {
		...
		for (var zone : zoneShapes) {
			stream = Stream.concat(stream, zone.value().getBlocks());
		}
	...
	public VoxelShape createVoxelShape() {
		...
		var shape = zoneShapes.getFirst().value().createVoxelShape();

		for (int i = 1; i < zoneShapes.size(); i++) {
			shape = Shapes.or(shape, zoneShapes.get(i).value().createVoxelShape());
		}
	...
	public VoxelShape createBlockRenderingShape(Predicate<BlockPos> predicate) {
		...
		var shape = zoneShapes.getFirst().value().createBlockRenderingShape(predicate);

		for (int i = 1; i < zoneShapes.size(); i++) {
			shape = Shapes.or(shape, zoneShapes.get(i).value().createBlockRenderingShape(predicate));
		}
	...
	public double closestDistanceTo(Vec3 pos) {
		...
		for (var shape : zoneShapes) {
			dist = Math.min(dist, shape.value().closestDistanceTo(pos));
	...
	public ZoneShape move(double x, double y, double z) {
		var shapes = new ArrayList<Ref<ZoneShape>>(zoneShapes.size());

		for (var shape : zoneShapes) {
			shapes.add(shape.value().move(x, y, z).ref());
		}

		var moved = box.move(x, y, z);
		return new JoinedZoneShape(shapes, moved, new AAIBB(Mth.floor(moved.minX), Mth.floor(moved.minY), Mth.floor(moved.minZ), Mth.ceil(moved.maxX), Mth.ceil(moved.maxY), Mth.ceil(moved.maxZ)));
	}
