...
public class FramebufferUtils {
	public static final int READ_FBO = ((VLDirectStateAccess) RenderSystem.getDevice()).vl$createFrameBufferObject();
	...
	public static NativeImage capture(RenderTarget framebuffer, int mipLevel, boolean flipX, boolean flipY) {
		...
		var buffer = device.createBuffer(() -> "Screenshot buffer", GpuBuffer.USAGE_MAP_READ | GpuBuffer.USAGE_COPY_DST, w * h * colorTexture.getFormat().pixelSize());

		GlStateManager.clearGlErrors();
		((VLDirectStateAccess) device).vl$directStateAccess().bindFrameBufferTextures(READ_FBO, ((GlTexture) colorTexture).glId(), 0, mipLevel, 36008);
		GlStateManager._glBindBuffer(GlConst.GL_PIXEL_PACK_BUFFER, buffer.vl$getHandle());
		...
		try (var view = device.createCommandEncoder().mapBuffer(buffer, true, false)) {
			var buf = view.data();
