...
public class BulkLevelModificationHolder implements BlockModificationConsumer {
	...
	public int apply(Level level, BulkLevelModification modification, boolean undoable) {
		...
		for (var sd : sections.values()) {
			if (!sd.modified) {
				if (debug) {
					VidLib.LOGGER.info("Skipped unmodified bulk edit section @ %d, %d, %d".formatted(sd.pos.x(), sd.pos.y(), sd.pos.z()));
				}
			...
			if (sd.levelChunk == null || sd.levelChunkSection == null) {
				if (debug) {
					VidLib.LOGGER.info("Skipped invalid bulk edit section @ %d, %d, %d".formatted(sd.pos.x(), sd.pos.y(), sd.pos.z()));
				}
			...
			if (sd.hasOnlyAir && sd.levelChunkSection.hasOnlyAir()) {
				if (debug) {
					VidLib.LOGGER.info("Skipped empty bulk edit section @ %d, %d, %d".formatted(sd.pos.x(), sd.pos.y(), sd.pos.z()));
				}
		...
		if (debug) {
			VidLib.LOGGER.info("Modified " + count + " blocks");
		}
