...
public interface ClientWaypoints {
	...
	static void draw(GuiGraphics graphics, DeltaTracker deltaTracker) {
		...
		var list = new ArrayList<ScreenWaypoint>(session.waypoints.size());

		for (var waypoint : session.waypoints.values()) {
			if (waypoint.enabled && waypoint.dimension == level.dimension()) {
		...
		for (var wp : list) {
			...
			var wpos = worldMouse.screen(wp.pos());
