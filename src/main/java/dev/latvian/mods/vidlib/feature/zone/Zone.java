...
public class Zone {
	...
	public void tick(@Nullable Level level) {
		...
		if (level != null) {
			var shape = volume.shape().value();

			for (var entity : shape.collectEntities(level, volume.entityFilter().value())) {
				if (entity.isAlive() && shape.intersects(entity.getBoundingBox())) {
