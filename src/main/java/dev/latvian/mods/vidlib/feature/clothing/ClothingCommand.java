			...
			.then(Commands.argument("player", EntityArgument.players())
				.then(Commands.argument("clothing", ResourceLocationArgument.id())
					...
					.then(Commands.argument("parts", ClothingParts.KNOWN_CODEC.argument(buildContext))
						.executes(ctx -> clothing(EntityArgument.getPlayers(ctx, "player"), new Clothing(ResourceKey.create(EquipmentAssets.ROOT_ID, ResourceLocationArgument.getId(ctx, "clothing")), ClothingParts.KNOWN_CODEC.get(ctx, "parts"))))
					)
				)
			)
		)
