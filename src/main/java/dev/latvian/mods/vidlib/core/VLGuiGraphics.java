...
public interface VLGuiGraphics {
	...
	default void healthBar(int x, int y, int w, int h, float hp) {
		var graphics = (GuiGraphics) this;
		graphics.fill(x, y, x + w, y + h, 0xFFFFFFFF);
		graphics.fill(x + 1, y + 1, x + w - 1, y + h - 1, 0xFF000000);
		var barColor = Color.hsb(KMath.lerp(hp, 0F, 0.35F), 0.71F, 1F, 255);
		graphics.fill(x + 2, y + 2, x + Mth.ceil((w - 3F) * hp + 1F), y + h - 2, barColor.argb());
	}

	default void healthBarWithLabel(Font font, Entity entity, int x, int y, int w, int h, boolean renderName, boolean renderHealth) {
		var graphics = (GuiGraphics) this;
		var mc = vl$mc();
		...
		graphics.drawString(font, name, -font.width(name) / 2F, -3F, 0xFFFFFFFF, true);
