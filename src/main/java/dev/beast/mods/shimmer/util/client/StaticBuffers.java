...
public record StaticBuffers(VertexFormat format, int vertexCount, int indexCount, GpuBuffer vertexBuffer, @Nullable IndexBuffer indexBuffer) {
	public void close() {
	...
	public static StaticBuffers of(MeshData meshData, @Nullable Supplier<String> name, @Nullable Optional<IndexBuffer> indexBuffer) {
		...
		var vertexBuffer = RenderSystem.getDevice().createBuffer(name, BufferType.VERTICES, BufferUsage.STATIC_WRITE, meshData.vertexBuffer());
		var indexBuf = indexBuffer == null ? IndexBuffer.of(meshData) : indexBuffer.orElse(null);
		return new StaticBuffers(format, meshData.drawState().vertexCount(), meshData.drawState().indexCount(), vertexBuffer, indexBuf);
	}
