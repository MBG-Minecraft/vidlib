...
public record Anchor(List<Area> areas, Map<ResourceKey<Level>, List<AAIBB>> shapes) {
	...
	public static final Anchor DEFAULT = new Anchor(List.of(), Map.of());
	public static Anchor client = DEFAULT;
	...
	).apply(instance, list -> list.isEmpty() ? DEFAULT : create(list)));
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, Anchor> STREAM_CODEC = CompositeStreamCodec.of(
		...
		list -> list.isEmpty() ? DEFAULT : create(list)
	);
